...
export function ProjectCaseStudy({ project }: ProjectCaseStudyProps) {
  ...
  const localized =
    projects.find((p) => p.slug === project.slug) ?? project;
  ...
  const prevProject = prevSlug
    ? projects.find((p) => p.slug === prevSlug)
    : null;
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-cream font-body text-ink">
      ...
      <div className="mx-auto mt-4 flex max-w-[960px] flex-wrap items-center justify-between gap-3 px-5 pb-7 max-[760px]:flex-col max-[760px]:items-stretch max-[760px]:px-3 max-[760px]:pb-5">
        ...
        >
          {prevSlug && prevProject ? (
            <Link href={`/projects/${prevSlug}`} className={issueNavLink}>
              {tProjects("prevIssue")}
            ...
            <span className={issueNavDisabled}>{tProjects("prevIssue")}</span>
          )}
          {nextSlug && nextProject ? (
            <Link href={`/projects/${nextSlug}`} className={issueNavLink}>
              {tProjects("nextIssue")}
              <span className="block text-xs opacity-75">
                {nextProject.title}
              </span>
            ...
            <span className={issueNavDisabled}>{tProjects("nextIssue")}</span>
          )}
        </nav>
      </div>
      ...
      <ComicStrip
        ...
        bang={caseStudy.bang}
        stripTitle={tProjects("stripTitle")}
      />
